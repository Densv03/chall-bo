...
export const AppRouter = () => {
    ...
    function renderAuthedUserRoutes(): React.ReactElement {
        return (<Routes>
            {[...AUTHED_USER_ROUTES, ...NOT_AUTHED_USER_ROUTES]
                .map(route => (<Route path={route.path}
                                      element={route.element}
                                      key={route.path}></Route>))
            }
            {AUTHED_USER_REDIRECTS.map(redirect => (
                <Route path={redirect.path} element={<Navigate
    ...
    function renderNotAuthedUserRoutes(): React.ReactElement {
        return (<Routes>
            {NOT_AUTHED_USER_REDIRECTS.map(redirect => (
                <Route path={redirect.path} element={<Navigate to={redirect.redirectTo}/>}
                       key={redirect.path}></Route>))}
            {NOT_AUTHED_USER_ROUTES.map(route => (<Route path={route.path}
                                                      element={route.element}
                                                      key={route.path}></Route>))}
    ...
    return (
        <div id="router-wrapper">{AuthService.isAuth() ? renderAuthedUserRoutes() : renderNotAuthedUserRoutes()}</div>
    );
};